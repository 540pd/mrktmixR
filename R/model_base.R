...
determine_vif_flag <- function(type, vif, vif_threshold) {
  if_else(sum(str_count(type, "fixed")) == 1 & is.infinite(vif), FALSE, vif > vif_threshold)
}
...
determine_expected_sign <- function(variable, pos_vars, neg_vars, var_agg_delimiter) {
  ...
  if_else(
    ...
    if_else(
      ...
      if_else(unlist(lapply(expected_neg, all)), FALSE, NA)
    )
  )
}
...
get_base_model <- function(lm_model, model_data, independent_var_info, pos_vars, neg_vars,
  ...
  loop_id <- 1
  model_coef_all <- tibble()
  model_smry_all <- tibble()
    ...
    model_coef <- independent_var_info %>%
      right_join(
        as.data.frame(lm_model_smry$coefficients) %>%
          ...
          mutate(vif = calculate_vif(lm_model)) %>%
          tibble(), by = "variable"
      ) %>%
      mutate(
        loop_id = loop_id,
      ...
      select(-.data$expected_sign)

    # Accumulate results
    model_coef_all <- bind_rows(model_coef_all, model_coef)
    model_smry_all <- bind_rows(model_smry_all, summarize_model(lm_model_smry, loop_id))
