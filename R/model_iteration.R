...
assemble_base_models <- function(candidate_predictors, model_indep_df, model_dep_df,
  ...
  independent_variable_info <- bind_rows(
    bind_rows(candidate_predictors[[1]], .id = "variable") %>%
      mutate(type = names(candidate_predictors[1])),
    bind_rows(candidate_predictors[[2]], .id = "variable") %>%
      mutate(type = names(candidate_predictors[2]))
  ) %>%
    bind_cols(sum = sapply(model_df_apl, sum, na.rm = TRUE))
  # %>%
  #   bind_rows(data.frame(variable = "(Intercept)", sum = nrow(model_df_apl), type = "intercept"))

  # Add intercept info if necessary
  if (with_intercept) {
    independent_variable_info <- bind_rows(independent_variable_info, tibble(variable = "(Intercept)", sum = nrow(model_df_apl), type = "intercept"))
  }

  # Combine dependent and independent data
  modeling_df <- bind_cols(model_dep_df, model_df_apl[rownames(model_df_apl) %in% rownames(model_dep_df),])
...
collate_base_models <- function(candidate_variables_list, model_df, model_dep_df,
  ...
  model_smry_all<-purrr::map_dfr(base_models, 2, .id = "model_id") %>%
    ...
    select(-.data$residuals_all)

  lm_model_all <- map(base_models, 3)
...
collate_models<-function(candidate_variables_list, model_df, dep_var_info,
  ...
  dep_apl_df_list[[1]] <- dep_apl_df_list[[1]] %>% rownames_to_column("dependent_id")
  ...
  model_coef_all <- dep_apl_df_list[[1]] %>%
    dplyr::full_join(map_dfr(model_result, 1, .id = "dependent_id") %>%
                mutate(contri = .data$Estimate * sum,
  ...
  model_smry_all <- map_dfr(model_result, 2, .id = "dependent_id")

  # Initial summarization of flags
  mdl_smry_flag <- model_coef_all %>%
    dplyr::group_by(.data$dependent_id, .data$model_id, .data$loop_id) %>%
    dplyr::summarise(
      # max_vif = max(vif, na.rm = TRUE), #intercept should be handled differently
  ...
  smry_var_type <- model_coef_all %>%
    ...
    tidyr::pivot_longer(
      cols = .data$contri:.data$pvalue_avg,
      names_to = "variable_name",
      values_to = "value"
    ) %>%
    mutate(variable = paste(.data$type, .data$variable_name, sep = "_")) %>%
    select(-.data$type, -.data$variable_name)
  ...
  mdl_smry_var <- model_coef_all %>%
    select(.data$dependent_id, .data$model_id, .data$loop_id, .data$variable, .data$contri, .data$contri_perc) %>%
    ...
    tidyr::pivot_longer(
      cols = .data$contri:.data$contri_perc,
      names_to = "variable_name",
      values_to = "value"
    ) %>%
    mutate(variable_new = paste(.data$variable, .data$variable_name, sep = "_")) %>%
    select(-.data$variable, -.data$variable_name)

  # Pivot wider and prepare for join
  mdl_smry_var_wide <- mdl_smry_var %>%
    tidyr::pivot_wider(names_from = .data$variable_new, values_from = .data$value)
  ...
  mdl_smry <- mdl_smry %>%
    mutate(flag_num = rowSums(across(c(.data$flag_pvalue, .data$flag_sign, .data$flag_vif)), na.rm = TRUE))
  ...
  lm_model_all <- map(model_result, 3)
