...
#'                                               delimiter = "\\|")
#' }
#' @importFrom dplyr mutate across any_of bind_cols select if_else
...
decompose_model_component <- function(variables_wt_weights, model_df,
  ...
  model_df_selected <- model_df %>%
    mutate(across(any_of(names(variables_wt_weights)),
                  ~ .x * variables_wt_weights[cur_column()]),
           .keep = "used")
  ...
  apl_info <- pmap(variable_info, function(variable, adstock, power, lag) {
  ...
  model_df_selected <- model_df_selected %>%
    bind_cols(model_df_transformed) %>%
    select(any_of(names(variables_wt_weights)))

  # Adjust for weight coefficients if not treating as coefficients
  if (!is_weight_coefficient) {
    model_df_selected <- model_df_selected %>%
      mutate(across(everything(), ~ .x / sum(.x, na.rm = TRUE) * variables_wt_weights[cur_column()]))
  }
